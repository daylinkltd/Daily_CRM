...
export async function loadMetrics(db: DB, workspaceId: string): Promise<MetricsBundle> {
  ...
  const [
    ...
    db.from('conversations').select('id', { count: 'exact', head: true }).eq('status', 'open').eq('workspace_id', workspaceId),
    db
      .from('conversations')
    ...
    db
      ...
      .gte('created_at', yesterdayStart)
      .lt('created_at', todayStart),
    db.from('deals').select('value, status').eq('status', 'open').eq('workspace_id', workspaceId),
    db
      .from('messages')
...
export async function loadPipelineDonut(db: DB, workspaceId: string): Promise<PipelineDonutData> {
  const [stagesRes, dealsRes] = await Promise.all([
    db
      ...
      .select('id, name, color, pipeline_id, position, pipelines!inner(workspace_id)')
...
export async function loadActivity(db: DB, workspaceId: string, limit = 20): Promise<ActivityItem[]> {
  ...
  const [msgs, contacts, deals, broadcasts, autoLogs] = await Promise.all([
    db
      .from('messages')
      .select('id, content_text, sender_type, created_at, conversation_id, conversations!inner(contact_id, workspace_id, contacts(name, phone))')
