...
function RateCell({
  ...
  value: number;
  total: number;
  /** Tailwind bg class for the fill, e.g. "bg-[#00aef0]" */
  color: string;
}) {
  ...
  return (
    <div className="space-y-6">
      ...
      {anySending && (
        ...
        >
          <div className="broadcast-indeterminate-bar h-0.5 bg-[#00aef0]" />
          <style jsx>{`
      ...
      {broadcasts.length === 0 ? (
        <div className="flex h-64 flex-col items-center justify-center rounded-xl border border-slate-800 bg-slate-900">
          ...
          <Button
            onClick={() => router.push('/broadcasts/new')}
            className="mt-4 bg-[#00aef0] text-white hover:bg-[#00aef0]"
          >
            <Plus className="h-4 w-4" />
            New Broadcast
          </Button>
