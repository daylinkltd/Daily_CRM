...
export default function POSTerminalPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] flex-col lg:flex-row gap-4 p-4 bg-slate-950 text-slate-100 overflow-hidden">
      {/* LEFT: Product Catalog & Search */}
      <div className="flex-1 flex flex-col gap-4 min-w-0">
        {/* Search Bar & Actions */}
        <div className="flex items-center gap-3 bg-slate-900/90 p-3.5 rounded-2xl border border-slate-800 backdrop-blur-md shadow-lg">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="pl-10 h-11 bg-slate-950 border-slate-800 text-white rounded-xl focus:border-[#00aef0] font-medium"
            />
          </div>
          <Button
            variant="outline"
            onClick={() => setShowHeldDrawer(true)}
            className="h-11 border-slate-800 bg-slate-950 text-amber-400 hover:bg-slate-900 gap-2 rounded-xl"
          >
          ...
          <Button
            ...
            className="h-11 border-slate-800 bg-slate-950 text-emerald-400 hover:bg-slate-900 gap-2 rounded-xl"
          >
        ...
        <div className="flex items-center justify-between bg-slate-900/50 p-2.5 rounded-xl border border-slate-800/80">
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400 font-bold px-2">Customer Type:</span>
          ...
          <div className="flex items-center gap-2">
            <Input
              ...
              className="h-8 w-36 bg-slate-950 border-slate-800 text-xs text-white rounded-lg"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 pr-1">
          {products.map((product) => (
            <div
              ...
              className="bg-slate-900/60 border border-slate-800 hover:border-[#00aef0]/50 rounded-2xl p-3 flex flex-col justify-between cursor-pointer transition-all hover:scale-[1.02] shadow-md group"
            >
              <div>
                <div className="text-xs font-bold text-white group-hover:text-[#00aef0] transition-colors line-clamp-2">
                  {product.name}
                </div>
                <div className="text-[11px] font-mono text-slate-500 mt-1">SKU: {product.sku}</div>
              </div>
      ...
      <div className="w-full lg:w-96 bg-slate-900/90 border border-slate-800 rounded-3xl p-4 flex flex-col justify-between shadow-2xl backdrop-blur-md">
        <div className="space-y-3 overflow-hidden flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1">
            {cart.length === 0 ? (
              ...
              cart.map((item) => (
                <div key={item.product_id} className="bg-slate-950 p-2.5 rounded-xl border border-slate-800/80 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="p-1 text-slate-400 hover:text-white text-[10px] font-mono bg-slate-900 border border-slate-800 rounded"
                    >
                      +S/N
                    </button>
                    <button onClick={() => updateQuantity(item.product_id, -1)} className="p-1 text-slate-400 hover:text-white">
        ...
        <div className="border-t border-slate-800 pt-3 space-y-3 mt-2">
          <div className="space-y-1.5 text-xs text-slate-400">
            ...
            <div className="flex justify-between">
              <span>GST Total</span>
              <span className="text-white font-semibold">₹{taxTotal.toFixed(2)}</span>
            </div>
              ...
              <button
                ...
                className={`py-2 rounded-xl text-[11px] font-bold transition-all flex items-center justify-center gap-1 ${
                  selectedPayment === method
                    ? "bg-[#00aef0] text-white shadow-lg"
          ...
          {(selectedPayment === "UPI" || selectedPayment === "SPLIT") && (
            <div className="bg-slate-950 p-2.5 rounded-xl border border-slate-800 space-y-1">
              ...
              <select
                ...
                className="w-full bg-slate-900 border border-slate-800 text-white rounded-lg text-xs h-8 px-2"
              >
          ...
          {selectedPayment === "SPLIT" && (
            <div className="bg-slate-950 p-2.5 rounded-xl border border-slate-800 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <Label className="text-[10px] text-slate-400">Cash Amount (₹)</Label>
                  <Input
                    type="number"
                    value={splitCashAmount || ""}
                    onChange={(e) => setSplitCashAmount(Number(e.target.value || 0))}
                    placeholder="0.00"
                    className="bg-slate-900 border-slate-800 text-white text-xs h-8 font-bold"
                  />
                </div>
                <div>
                  <Label className="text-[10px] text-slate-400">UPI Amount (₹)</Label>
                  <Input
                    type="number"
                    value={splitUpiAmount || ""}
                    onChange={(e) => setSplitUpiAmount(Number(e.target.value || 0))}
                    placeholder="0.00"
                    className="bg-slate-900 border-slate-800 text-emerald-400 text-xs h-8 font-bold"
                  />
                </div>
              </div>
            </div>
          ...
          <Button
            ...
            className="w-full bg-[#00aef0] hover:bg-[#0284c7] text-white font-bold h-12 rounded-xl shadow-lg shadow-[#00aef0]/20 text-sm"
          >
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setShowHeldDrawer(false)} className="text-slate-400 hover:text-white">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {heldBills.length === 0 ? (
                ...
                heldBills.map((held) => (
                  <div key={held.id} className="p-3 bg-slate-950 border border-slate-800 rounded-xl flex items-center justify-between">
                    <div>
                      <div className="font-bold text-white text-xs">{held.hold_number}</div>
                      <div className="text-[11px] text-slate-400">{held.items.length} item(s) • Held at {held.created_at}</div>
                    </div>
                    <Button size="sm" onClick={() => handleRecallBill(held)} className="bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs rounded-lg gap-1">
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setShowShiftCloseModal(false)} className="text-slate-400 hover:text-white">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              {(["500", "200", "100", "50", "20", "10", "coins"] as const).map((denom) => (
                <div key={denom} className="space-y-1">
                  <Label className="text-[11px] text-slate-400 uppercase">₹{denom} Count</Label>
                  <Input
                    type="number"
            ...
            <div className="pt-3 border-t border-slate-800 flex items-center justify-between">
              <div>
                <div className="text-xs text-slate-400">Total Counted Cash</div>
                <div className="text-xl font-extrabold text-emerald-400">₹{totalCountedCash.toFixed(2)}</div>
              </div>
              <Button onClick={() => { toast.success("Shift closed & Z-Report generated!"); setShowShiftCloseModal(false); }} className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl h-10 px-4 text-xs">
                Close Shift
              </Button>
            </div>
          </div>
      ...
      {showSerialModal && selectedItemForSerial && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-sm w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h2 className="text-sm font-bold text-white">Add Serial / IMEI Number</h2>
              <button onClick={() => setShowSerialModal(false)} className="text-slate-400 hover:text-white">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="space-y-2">
              <Label className="text-xs text-slate-400">{selectedItemForSerial.name}</Label>
              <Input
                type="text"
            ...
            <Button onClick={handleSaveSerial} className="w-full bg-[#00aef0] text-white font-bold h-10 text-xs rounded-xl">
              Save Serial Number
            </Button>
          </div>
        </div>
