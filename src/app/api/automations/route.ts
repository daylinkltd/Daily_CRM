...
export async function POST(request: Request) {
  ...
  const { name, description, trigger_type, trigger_config, is_active, steps, template, workspace_id } = body
  ...
  const maxAutomations = wsData?.plan_limits?.max_automations as number | null
  if (maxAutomations !== null && maxAutomations !== undefined) {
    ...
    if (count !== null && count >= maxAutomations) {
      return NextResponse.json(
        { error: `Automation limit reached for this workspace. (Max: ${maxAutomations})` },
        { status: 403 }
      )
    }
  }
