...
export function ProjectTimeline({ projectId }: ProjectTimelineProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      // Fetch tasks that have dates
      const { data: tasksData } = await supabase
        .from('tasks')
        .select('id, title, start_date, due_date, epic_id, status_id, status, priority')
  ...
  if (orphanedTasks.length > 0) {
    groupedTasks.push({ id: 'no-epic', name: 'No Epic', tasks: orphanedTasks });
  }
  ...
  return (
    <Card className="border-border shadow-sm flex flex-col h-[600px]">
      ...
      <div className="flex-1 overflow-auto relative flex">
        ...
        <div className="w-[250px] border-r flex-shrink-0 bg-card sticky left-0 z-20">
          ...
          <div className="py-2">
            {groupedTasks.length === 0 ? (
              <div className="p-4 text-sm text-muted-foreground text-center">No scheduled tasks.</div>
            ) : (
        ...
        {/* Timeline Grid */}
        <div className="flex-1 overflow-x-auto relative min-w-[800px] bg-slate-50 dark:bg-slate-900">
          {/* Header */}
          <div className="h-[60px] border-b flex bg-muted/30 sticky top-0 z-10">
            {columns.map((col, i) => (
              <div key={i} className="flex-1 border-r min-w-[60px] flex flex-col justify-center items-center px-1">
                <span className="text-xs font-medium truncate w-full text-center">{col.label}</span>
          ...
          {/* Today Marker */}
          <div 
            className="absolute top-[60px] bottom-0 w-0.5 bg-red-500 z-10 opacity-50 pointer-events-none" 
            style={{ 
          ...
          <div className="py-2 relative">
            {groupedTasks.map(group => (
              <div key={`grid-${group.id}`} className="mb-2">
                <div className="h-[32px]"></div> {/* Epic spacer */}
                {expandedEpics[group.id] && group.tasks.map(t => (
                  <div key={`grid-task-${t.id}`} className="h-10 relative group border-b border-border/30 hover:bg-muted/10">
                    <div 
                      className="absolute top-1.5 h-7 bg-primary text-primary-foreground text-xs rounded shadow-sm px-2 flex items-center overflow-hidden whitespace-nowrap cursor-pointer hover:brightness-110 transition-all z-10"
            ...
            <div className="absolute inset-0 flex pointer-events-none opacity-20">
              {columns.map((_, i) => (
                <div key={`bg-${i}`} className="flex-1 border-r min-w-[60px] h-full" />
              ))}
            </div>
          </div>
        </div>
