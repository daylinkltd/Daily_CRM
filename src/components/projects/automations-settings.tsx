                  ...
                  <Select value={actionMemberId} onValueChange={(val) => setActionMemberId(val || '')}>
                    ...
                    <SelectContent>
                      ...
                      {members.map(m => {
                        ...
                        return <SelectItem key={m.id} value={m.id}>{prof?.full_name || 'Unknown'}</SelectItem>;
                      })}
                    </SelectContent>
                  </Select>
                )}
        ...
        {automations.map(rule => {
          ...
          if (rule.action_type === 'ASSIGN_MEMBER') {
            ...
            if (memberId === 'none') {
               ...
               const prof = member ? (Array.isArray(member.profiles) ? member.profiles[0] : member.profiles) : null;
               actionText = `Assign to ${prof?.full_name || 'Unknown'}`;
            }
