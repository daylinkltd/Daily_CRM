...
export function ProfileForm() {
  ...
  const [workspaceRoleName, setWorkspaceRoleName] = useState<string | null>(null);

  // Fetch workspace role name
  useEffect(() => {
    ...
    fetchRole();
  }, [user?.id, activeWorkspace?.id, supabase]);
  ...
  const dirty =
    !profile 
      ? true // Always allow saving if profile is missing, validation will catch empty fields
      ...
      <CardContent>
        <form onSubmit={onSubmit} className="space-y-6">
          ...
          <div className="flex justify-end">
            <Button type="submit" disabled={saving || !dirty || (!profile && !user)}>
              {saving ? (
                <>
